var merchandiseManagement = function() {
    ...
    //batch modify hashtag must correct
    Then(/batch modify hashtag must correct/, function(next) {
        var oHtags, stand, nHtag;
        stand = this.stand;
        nHtag = 'dummy';

        this.stand.getFirstRowItemHashtag().then(
            function(htags) {
                oHtags = htags;
            }
        ).then(
            function() {
                //set new hashtag
                stand.goBatchHtagModify(nHtag).then(
                    function(flag) {
                        expect(flag, 'batch hashtag modify fail').to.be.true;
                    }
                );
            }
        ).then(
            function() {
                //recover
                stand.goBatchHtagModify(oHtags).then(
                    function(flag) {
                        expect(flag, 'batch hashtag modify fail').to.be.true;
                    }
                );

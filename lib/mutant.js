var mutant = {
    createItem: function(type, data) {
        var selectType = require(__base + constants.PO.selectType),
            neoData = {},
            removes = ['mid', 'payType', 'barCode', 'itemNumber', 'itemLocation', 'durationDay', 'customCategory', 'promotePrice', 'shipFee', 'shipType', 'imageAmount', 'specs', 'owner', 'bargainSwitch', 'bargainRejectPrice', 'hashtags'];
        ...
        return selectType.go().then(
            ...
            function(singleEdit) {
                if (!neoData.hashtags) return singleEdit;
                else {
                    return singleEdit.fillHashTags(neoData.hashtags);
                }//end if
            }
        ).then(
            function(singleEdit) {
            ...
            function(singleEdit) {
                // if (!neoData.bargainSwitch) return singleEdit;
                // return singleEdit.turnBargainAs(neoData.bargainSwitch);
                return singleEdit.turnBargainAs(neoData.bargainSwitch || '0');
            }
        ).then(
            function(singleEdit) {
                if (!neoData.bargainRejectPrice) return singleEdit;
                return singleEdit.fillBargainRejectPrice(neoData.bargainRejectPrice);
            }
        ).then(
            function(singleEdit) {

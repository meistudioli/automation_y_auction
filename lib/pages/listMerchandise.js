...
listMerchandise = function() {
    ...
    this.selector = {
        ...
        hashtags: '.hashtags',
        trigger4BatchHtag: 'a[data-info="htag"]',
        batchHtagDialog: '.msg-dialog .batchHtag',
        batchHashtag: 'hash-tag-t',
        batchHtagDialogSubmit: '.batchHtag input[type="submit"]',
        sign4Pjax: '.pjax .loading'
    };
...
listMerchandise.prototype.offshelfFirstRowItemBK = function() {
...
listMerchandise.prototype.offshelfFirstRowItem = function() {
	var stand = this, operate, item;

	item = this.one('itemList');
	return item.isPresent().then(
		function() {
			return item.$('.publish p').getText().then(
				function(itemType) {
					itemType = common.trim(itemType);
...
listMerchandise.prototype.onshelfFirstRowItem = function() {
	var stand = this, flag = true, confirm, item;
...
listMerchandise.prototype.filterByStatus = function(status) {
	...
	return this.clickAndWaitUntilRedirect(this.all('filterItemStatus').get(idx)).then(
		function() {
			return stand;
		}
	);
...
listMerchandise.prototype.modifyFirstRowTitle = function(key) {
	...
	return this.getFirstRowItemTitle().then(
		...
		function() {
			var item;
			item = stand.one('itemList');
			return item.isPresent().then(
				function() {
					var flexedit;
					target = item;
					flexedit = item.$(stand.selector.itemTitleFlexEdit);
					return flexedit.isPresent().then(
						function() {
		...
		function() {
			...
			browser.getCurrentUrl().then(
				...
				function() {
					var itemPage, imgLink;
					imgLink = target.$(stand.selector.imgLink);
					return imgLink.isPresent().then(
						function() {
...
listMerchandise.prototype.modifyFirstRowPrice = function(key) {
	...
	return this.getFirstRowItemPrice().then(
		...
		function() {
			var item = stand.one('itemList');
			return item.isPresent().then(
				function() {
					var flexedit = item.$(stand.selector.itemPriceFlexEdit);
					target = item;
					return flexedit.isPresent().then(
						function() {
							return browser.actions().mouseMove(target).perform().then(
...
listMerchandise.prototype.pickAllItems = function() {
	var stand = this, unit;
	unit = this.one('pickAll');
	return unit.isPresent().then(
		function() {
			unit.click();
		}
	).then(
...
listMerchandise.prototype.getFirstRowItemStockBK = function() {
...
listMerchandise.prototype.getFirstRowItemStock = function() {
	var stand = this, value, hasSpec = false, set = 1, target, item;
	...
	return item.isPresent().then(
		function() {
			var flexedit;
			target = item;
			flexedit = item.$(stand.selector.itemStockFlexEdit);
			return flexedit.isPresent().then(
				function() {
					return browser.actions().mouseMove(target).perform().then(
						function() {
...
listMerchandise.prototype.modifyFirstRowStock = function(key) {
	var stand = this, flag = true, original, target, hasSpec = false, set = 1, singleEdit, item;
...
listMerchandise.prototype.getFirstRowItemHashtag = function() {
	var stahs = this, htags = [];
	return this.one('hashtags').$$('a').each(
        function(unit, idx) {
            unit.getText().then(
                function(text) {
                    htags.push(text);
		...
		function() {
			return htags.map(function(e){return e.trim();}).sort().join(',');
		},
		function() {
			return [];
		}
...
listMerchandise.prototype.goBatchHtagModify = function(htag) {
	var stand = this, flag = true;
	return this.pickAllItems().then(
		function(stand) {
			stand.one('trigger4BatchHtag').click().then(
				function() {
					stand.waitUntilPresent('batchHtagDialog');
				}
			);
		}
	).then(
		function() {
            browser.executeScript(
                function(hashtag, htag) {
                    hashtag.value = htag;
                }
            , stand.one('batchHashtag').getWebElement(), htag.split(',')).then(
            	function() {
            		stand.one('batchHtagDialogSubmit').click();
            	}
            ).then(
            	function() {
            		stand.waitUntilPresent('messageBar').then(
            			function() {
            				stand.waitUntilPresent('sign4Pjax');
            			}
            		);
            	}
            ).then(
            	function() {
            		stand.waitUntilNotPresent('sign4Pjax');
            	}
            );
		}
	).then(
		function() {
			...
			browser.getCurrentUrl().then(
				...
				function() {
					var itemPage, imgLink;
					imgLink = stand.one('itemList').$(stand.selector.imgLink);
					return imgLink.isPresent().then(
						function() {
							return stand.clickAndWaitUntilRedirect(imgLink).then(
								function() {
				...
				function(htags) {
					browser.get(cUrl).then(
						function() {
							if (htag != htags) throw new Error('batch hashtag hashtag modify fail');
						}
					);
				}
			);

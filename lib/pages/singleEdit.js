...
singleEdit = function() {
    ...
    this.selector = {
        ...
        'payType711(pockii)': 'input[name=".pockii_711cvs"]',
        payTypeCup: 'input[name=".pockii_cup"]',
        payTypePockii: 'input[name=".pockii"]',
        ...
        sign4illuTransUpload: '.illuTrans li.upload',
        ...
        hashtag: 'hash-tag-t',
        rteReady: '#editor-iframe',
        rteHTMLMode: '#literalMode'
    };
};
...
singleEdit.prototype.fillRequestData = function(data) {
    var stand = this;
    return this.one('body').isPresent().then(
        function() {
            for (var i in data) {
                if (typeof stand.selector[i] == 'undefined') continue;
                if (i == 'itemDesc') {
                    stand.waitUntilPresent('rteReady').then(
                        function() {
                            stand.one('rteHTMLMode').click();
                        }
                    );
                }//end if
    ...
    return this.rollTo('illuTransWrap').then(
        function() {
            for (var i=-1,l=amount;++i<l;) {
                stand.one('illuTransUnit').isPresent().then(
                    function() {
                        stand.one('illuTransInput').sendKeys(stand.attachment.image).then(
                            function() {
                                browser.wait(
                                    function() {
                                        return stand.one('sign4illuTransUpload').isPresent().then(
                                            function(flag) {
                                                return !flag;
                                            }
                                        );
...
singleEdit.prototype.fillSpecQty = function(amount, barCode, itemNumber) {
    ...
    , this.one('specQtyShow').getWebElement()).then(
        function() {
...
singleEdit.prototype.hideStepBar = function(mode) {
    var method, stand;
    method = (mode) ? 'add' : 'remove';
    stand = this;
    return this.one('stepBar').isPresent().then(
        function(flag) {
            if (flag) {
                browser.executeScript(
                    function(stepBar, method) {
                        stepBar.classList[method]('hide');
                    }
                , stand.one('stepBar').getWebElement(), method);
            }//end if
        }
    );
};
...
singleEdit.prototype.fillSingleShipping = function(types, shipFee, ships) {
    ...
    //pay-type
    return this.hideStepBar(true).then(
        function() {
            stand.all('payTypeWrap').each(
                function(input) {
        ...
        function() {
            var shipTypeSet;
            shipTypeSet = stand.all('shipTypeSet');

            shipType.forEach(
                function(type, idx) {
                    shipTypeSet.get(idx).isPresent().then(
                        function() {
                            var ship;
                            ship = shipTypeSet.get(idx);
...
singleEdit.prototype.fillHashTags = function(htag) {
    var stand = this;
    return stand.one('hashtag').isPresent().then(
        function(flag) {
            if (!flag) throw new Error('<hash-tag-t> not support.');
            stand.rollTo('hashtag').then(
                function() {
                    browser.executeScript(
                        function(hashtag, htag) {
                            hashtag.value = htag;
                        }
                    , stand.one('hashtag').getWebElement(), htag);
                }
            );
        }
    ).thenCatch(
